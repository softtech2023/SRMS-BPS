...
Public Class frmSearchStudeEncodeGrades
    ...
    Private Sub frmSearchStudeEncodeGrades_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            str = "select p.StudentNumber, concat(ss.LName,', ',ss.FName,' ',ss.MName) as StudeName from studeprofile p join semesterstude ss on p.StudentNumber=ss.StudentNumber where ss.Semester='" & frmParent.lblSem.Text & "' and ss.SY='" & frmParent.lblSY.Text & "' group by p.StudentNumber order by ss.LName"
            conn.Open()
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        Try
            str = "select p.StudentNumber, concat(ss.LName,', ',ss.FName,' ',ss.MName) as StudeName from studeprofile p join semesterstude ss on p.StudentNumber=ss.StudentNumber where ss.LName like '%" & txtSearch.Text & "%' and ss.Semester='" & frmParent.lblSem.Text & "' and ss.SY='" & frmParent.lblSY.Text & "' group by p.StudentNumber order by ss.LName"
            conn.Open()
            Dim mysda As New MySqlDataAdapter(str, conn)
            Dim ds As New DataSet
            mysda.Fill(ds, "studeprofile, semesterstude")
