    ...
    Private Sub btnStudeReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnStudeReport.Click
        ...
        Try
            ...
            If cboStudeReport.SelectedIndex = 0 Then
                str = "Select p.StudentNumber, concat(p.FirstName,' ',p.MiddleName,' ',p.LastName) as StudentName, p.Course, s.SY, s.AcctTotal, s.TotalPayments, s.CurrentBalance from studeprofile p join studeaccount s on p.StudentNumber=s.StudentNumber where s.StudentNumber='" & mskStudentNumber.Text & "' and s.SY='" & frmParent.lblSY.Text & "' group by s.StudentNumber, s.SY"
                conn.Open()
            ...
            If cboStudeReport.SelectedIndex = 1 Then
                ...
                str = "select s.StudentNumber, concat(s.FirstName,' ',s.MiddleName,' ',s.LastName) as StudentName, s.AcctTotal, s.CurrentBalance, s.PaymentMode, s.Sem, s.SY, pa.PDate, pa.ORNumber, pa.Amount, pa.description, pa.PaymentType, pa.ORStatus, pa.Sem, pa.SY from studeaccount s join paymentsaccounts pa on s.StudentNumber=pa.StudentNumber where s.StudentNumber='" & mskStudentNumber.Text & "' and s.SY='" & frmParent.lblSY.Text & "' and pa.SY='" & frmParent.lblSY.Text & "' and pa.ORStatus='Valid' and pa.CollectionSource!='Services' group by pa.ORNumber"
                conn.Open()
                Dim mysDA As New MySqlDataAdapter(Str, conn)
            ...
            If cboStudeReport.SelectedIndex = 2 Then
                str = "select s.StudentNumber, concat(s.FirstName,' ',s.MiddleName,' ',s.LastName) as StudentName, s.Sem, s.SY, pa.PDate, pa.ORNumber, pa.Amount, pa.description, pa.PaymentType, pa.ORStatus, pa.Sem, pa.SY, s.Course, s.YearLevel from studeaccount s join paymentsaccounts pa on s.StudentNumber=pa.StudentNumber where s.StudentNumber='" & mskStudentNumber.Text & "' and s.SY='" & frmParent.lblSY.Text & "' and pa.SY='" & frmParent.lblSY.Text & "' and pa.ORStatus='Valid' and pa.CollectionSource='Services' group by pa.ORNumber"
                conn.Open()
                Dim mysDA As New MySqlDataAdapter(str, conn)
                Dim mysds As New DataSet
                mysDA.Fill(mysds, "studeprofile, paymentsaccounts")
            ...
            If cboStudeReport.SelectedIndex = 4 Then
                str = "select * from studeaccount where StudentNumber='" & mskStudentNumber.Text & "' and SY='" & frmParent.lblSY.Text & "' order by FeesDesc"
                conn.Open()
                Dim mysDA As New MySqlDataAdapter(str, conn)
                Dim mysds As New DataSet
